...
// 输入当前状态，输出推力u
Eigen::VectorXd RokaeMPC::compute_control(
    ...
    Eigen::VectorXd ref_state = Eigen::VectorXd::Zero(n);
    ref_state.head(n_dof) = q_ref;
    ref_state.tail(n_dof).setZero();
    ...
    Eigen::VectorXd error_state = current_state - ref_state;

    // U_K 现在的维度应该是 p * N
    Eigen::VectorXd U_K = Prediction(error_state, result.E, result.H, N, p);
